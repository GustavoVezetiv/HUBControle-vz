...
const VEZETIV_SIGNATURE_HREF = "https://links.vezetiv.dev";
const VEZETIV_SIGNATURE_LABEL = "Ecossistema Vezetiv.dev";
const VEZETIV_SIGNATURE_LABEL_UPPERCASE = VEZETIV_SIGNATURE_LABEL.toUpperCase();
const VEZETIV_SIGNATURE_COLOR = "#7BB8D9";
const VEZETIV_SIGNATURE_ICON_PATH =
...
function buildVezetivSignatureSvg() {
  const width = 390;
  const height = 54;
  const iconSize = 24;
  const iconX = 24;
  ...
  <defs>
    <filter id="vezetivGlow" x="-80%" y="-80%" width="260%" height="260%">
      <feDropShadow dx="0" dy="0" stdDeviation="2" flood-color="${VEZETIV_SIGNATURE_COLOR}" flood-opacity="0.55"/>
      <feDropShadow dx="0" dy="0" stdDeviation="5" flood-color="${VEZETIV_SIGNATURE_COLOR}" flood-opacity="0.25"/>
    </filter>
  </defs>
  <g fill="${VEZETIV_SIGNATURE_COLOR}" filter="url(#vezetivGlow)">
    <svg x="${iconX}" y="${iconY}" width="${iconSize}" height="${iconSize}" viewBox="0 0 512 512" aria-hidden="true">
      <path d="${VEZETIV_SIGNATURE_ICON_PATH}"/>
    </svg>
  </g>
  <text x="${labelX}" y="31" fill="${VEZETIV_SIGNATURE_COLOR}" font-family="Inter, Arial, Helvetica, sans-serif" font-size="13" font-weight="600" letter-spacing="3.9" text-transform="uppercase">${VEZETIV_SIGNATURE_LABEL_UPPERCASE}</text>
</svg>`;
}

function renderVezetivSignatureImage(): Promise<PdfSignatureImage | null> {
  if (typeof window === "undefined") {
    return Promise.resolve(null);
  }

  return new Promise((resolve) => {
    const svg = buildVezetivSignatureSvg();
    ...
    image.onload = () => {
      try {
        ...
        console.error("Erro ao preparar assinatura Vezetiv para PDF", error);
        resolve(null);
      } finally {
...
function ReimbursementReportModal({
  ...
  const [pdfSignatureImage, setPdfSignatureImage] = useState<PdfSignatureImage | null>(null);

  useEffect(() => {
    let active = true;

    renderVezetivSignatureImage().then((image) => {
      if (active) {
        setPdfSignatureImage(image);
      }
    });
      ...
      <div className="reimbursement-report-shell space-y-5">
        ...
        <article className="reimbursement-report rounded-lg border border-ink-950/10 bg-white p-6 text-ink-950 shadow-sm">
          ...
          <footer className="report-footer mt-8 flex flex-wrap items-center justify-between gap-2 border-t border-ink-950/10 pt-4 text-xs leading-5 text-ink-600">
            ...
            <VezetivSignatureWrapper className="flex justify-end" size="sm" />
          </footer>
        </article>
      </div>
    </Modal>
...
function drawVezetivPdfSignature(doc: jsPDF, pageWidth: number, pageHeight: number, signatureImage: PdfSignatureImage | null) {
  if (signatureImage) {
    const width = 82;
    const height = width * (signatureImage.heightPx / signatureImage.widthPx);
    const x = pageWidth - width - 10;
    const y = pageHeight - height - 4;

    doc.addImage(signatureImage.dataUrl, "PNG", x, y, width, height);
    doc.link(x, y, width, height, { url: VEZETIV_SIGNATURE_HREF });
    return;
  }

  const color: [number, number, number] = [123, 184, 217];
  const x = pageWidth - 92;
  const y = pageHeight - 8;

  doc.setFont("helvetica", "bold");
  doc.setFontSize(8);
  doc.setTextColor(...color);
  doc.text(VEZETIV_SIGNATURE_LABEL_UPPERCASE, x, y);
  doc.link(x, y - 5, 82, 8, { url: VEZETIV_SIGNATURE_HREF });
}

function drawReimbursementPdfFooter(doc: jsPDF, pageWidth: number, pageHeight: number, footerText: string, signatureImage: PdfSignatureImage | null) {
  doc.setFont("helvetica", "normal");
  doc.setFontSize(7);
  doc.setTextColor(100, 116, 139);
  doc.text(footerText, 10, pageHeight - 7);
  drawVezetivPdfSignature(doc, pageWidth, pageHeight, signatureImage);
}
...
function buildReimbursementPdfDocument({
  ...
  pdfSignatureImage: PdfSignatureImage | null;
  reimbursements: ReimbursementRow[];
  ...
  if (reimbursements.length === 0) {
    ...
    groups.forEach((group, groupIndex) => {
      ...
      autoTable(doc, {
        startY: cursorY,
        margin: { left: marginX, right: marginX, bottom: marginBottom + 14 },
  ...
  for (let pageNumber = 1; pageNumber <= totalPages; pageNumber += 1) {
    ...
    drawReimbursementPdfFooter(doc, pageWidth, pageHeight, footerText, pdfSignatureImage);
  }
