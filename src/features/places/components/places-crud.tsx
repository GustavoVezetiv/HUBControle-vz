                        ...
                        <div className="flex justify-end gap-2">
                          {buildPlaceMapUrl(item) ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="hub-action hub-action-secondary inline-flex items-center justify-center rounded-md border border-ink-950/10 px-4 py-2.5 text-sm font-semibold text-ink-950 transition hover:border-mint-500 hover:text-mint-600 dark:border-white/10 dark:text-slate-100"
                            >
                              Abrir no mapa
                            </a>
                          ) : null}
  ...
  return (
    <Modal title={modal?.mode === "edit" ? "Editar lugar" : "Novo lugar"} description="Use para acompanhar ideias de rolês, lugares visitados e experiências que merecem avaliação." onClose={onClose}>
      <form className="grid gap-4 md:grid-cols-2" onSubmit={(event) => { event.preventDefault(); onSubmit(submitValues); }}>
        ...
        {isVisited ? (
          <>
            <div className="hub-card md:col-span-2 rounded-md border border-mint-500/20 bg-mint-50 px-4 py-3 text-sm text-ink-700 dark:border-mint-400/20 dark:text-slate-200">
